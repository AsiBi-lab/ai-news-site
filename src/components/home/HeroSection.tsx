...
export function HeroSection() {
  ...
  const getContainerVars = () => {
    if (animContext === 'reduced') return reducedMotionVariants
    if (animContext === 'mobile') return mobileContainerVariants
    return containerVariants
  }

  const getItemVars = () => {
    if (animContext === 'reduced') return reducedMotionVariants
    if (animContext === 'mobile') return mobileItemVariants
    return itemVariants
  }

  const getStatVars = () => {
    if (animContext === 'reduced') return reducedMotionVariants
    if (animContext === 'mobile') return mobileStatVariants
    return statVariants
  }
  ...
  return (
    <section className="relative z-10 py-20 px-6">
      <motion.div
        ...
        variants={getContainerVars()}
        initial="hidden"
        animate="visible"
      >
        {/* Tagline */}
        <motion.div variants={getItemVars()}>
          <p className="text-sm font-medium tracking-genesis text-genesis-muted uppercase mb-6">
            STOP WASTING TIME ON AI TOOL RESEARCH
          </p>
        </motion.div>

        {/* 3D Title - pulse effect only on desktop */}
        <motion.h1
          variants={getItemVars()}
          className="mb-4"
        >
          {reducedAnimations ? (
            // Static title on mobile/reduced motion
            <span className="title-3d inline-block">AI DECK</span>
          ) : (
        ...
        >
          <motion.div
            whileHover={reducedAnimations ? undefined : { scale: 1.02 }}
            whileTap={reducedAnimations ? undefined : { scale: 0.98 }}
          ...
          <motion.p
            ...
            animate={{ opacity: 1 }}
            transition={reducedAnimations ? { duration: 0 } : { delay: 1.2, duration: 0.5 }}
          >
            Free forever - No signup required
          </motion.p>
        </motion.div>

        {/* Trusted By - simplified on mobile */}
        <motion.div
          variants={getItemVars()}
          className="mt-16"
        >
          ...
          <div className="flex items-center justify-center gap-8 flex-wrap">
            {trustedBy.map((company, index) => (
              reducedAnimations ? (
                ...
                <span
                  key={company}
                  className="text-genesis font-semibold text-sm opacity-60"
                >
                  {company}
                </span>
              ) : (
      ...
      {/* Stats Section - simplified animations on mobile */}
      <motion.div
        className="max-w-4xl mx-auto mt-16 px-6"
        initial={{ opacity: reducedAnimations ? 1 : 0, y: reducedAnimations ? 0 : 40 }}
        animate={{ opacity: 1, y: 0 }}
        transition={reducedAnimations ? { duration: 0 } : { delay: 0.8, duration: 0.6, ease: EASE_SMOOTH }}
      >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((stat, index) => (
              <motion.div
                ...
                variants={getStatVars()}
                initial="hidden"
                animate="visible"
                transition={reducedAnimations ? { duration: 0 } : { delay: 1 + index * 0.15 }}
                whileHover={reducedAnimations ? undefined : { scale: 1.05 }}
              >
                {reducedAnimations ? (
                  // Static on mobile
